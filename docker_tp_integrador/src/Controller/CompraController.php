...
class CompraController extends AbstractController
{
    ...
    public function new(Request $request, CompraRepository $compraRepository, ClienteRepository $clienteRepository): Response
    {
        $compra = new Compra();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $compra->setImporteCompra($compra->getPrecioUnitario() * $compra->getCantidad());
            $compraRepository->save($compra, true);

            $compra->getCliente()->setSaldo( $compra->getCliente()->getSaldo() - $compra->getImporteCompra() );

            $clienteRepository->save($compra->getCliente(), true);
